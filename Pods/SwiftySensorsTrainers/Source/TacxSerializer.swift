...
open class TacxSerializer {
    ...
    public struct GeneralFEResult {
        public var speedKPH: Double = 0
    }
    
    public struct TrainerDataResult {
        public var instantaneousPower: UInt = 0
    }
    ...
    public static func readGeneralFE(_ data: Data) -> GeneralFEResult {
        let bytes = data.map {$0}
        
        var result = GeneralFEResult()
        result.speedKPH = Double(UInt16(bytes[4]) | UInt16(bytes[5]) << 8) * 0.001 * 3.6
        return result
    }
    
    public static func readTrainerData(_ data: Data) -> TrainerDataResult {
        let bytes = data.map {$0}
        var result = TrainerDataResult()
        result.instantaneousPower = UInt(UInt16(bytes[5]) | UInt16(bytes[6] << 4) << 4)
        return result
    }
